...
if [ -f "$STATE_PATH" ] && verified_cdp_endpoint "$PORT"; then
  ...
  CURRENT_INJECTOR_LOWER="$(printf '%s' "$INJECTOR" | /usr/bin/tr '[:upper:]' '[:lower:]')"
  SAVED_INJECTOR_LOWER="$(printf '%s' "$SAVED_INJECTOR" | /usr/bin/tr '[:upper:]' '[:lower:]')"
  if [ "$SAVED_SKIN_VERSION" = "$SKIN_VERSION" ] && [ "$SAVED_INJECTOR_LOWER" = "$CURRENT_INJECTOR_LOWER" ]; then
