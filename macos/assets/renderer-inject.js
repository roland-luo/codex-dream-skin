((cssText, artDataUrl, themeConfig) => {
  ...
  const ensure = () => {
    ...
    if (!chrome || chrome.parentElement !== document.body || chrome.dataset.dreamSkinVersion !== VERSION) {
      chrome?.remove();
      chrome = document.createElement("div");
      chrome.id = CHROME_ID;
      chrome.setAttribute("aria-hidden", "true");
      chrome.dataset.dreamSkinVersion = VERSION;
      chrome.innerHTML = `
        <div class="dream-skin-brand">
          <span class="dream-skin-brand-art-frame"><img class="dream-skin-brand-art" alt="" /></span>
          <span><b></b><small></small></span>
        </div>
        <div class="dream-skin-status"><i></i><span></span></div>
        <div class="dream-skin-quote"></div>
        <div class="dream-skin-polaroid">
          <img alt="" />
          <span></span>
        </div>`;
      document.body.appendChild(chrome);
    }
    chrome.querySelector(".dream-skin-brand-art").src = artUrl;
    ...
    chrome.querySelector(".dream-skin-polaroid img").src = artUrl;
    chrome.querySelector(".dream-skin-polaroid span").textContent = THEME.quote || "MAKE SOMETHING WONDERFUL";
  ...
  window[STATE_KEY] = {
    ...
    themePreset: THEME.preset || "portal",
    detectShellMode,
  };
  ensure();
